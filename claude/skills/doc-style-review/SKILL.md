...
- **Banned constructions**: the list in `CLAUDE.md` § Documentation, plus the full catalogue at
  ...
  for the pattern, not just the literal words — a paraphrase still counts.
- **Bare id citations**: any reference to a decision, requirement, ADR, probe, outcome, or named
  ...
  (`CLAUDE.md` § Communication — this rule already covers docs, not just chat, it just needs
  applying here too).
...
- Review-first: do not edit files unless asked. Report and let the human decide — same posture as
  `nw-documentarist` and this repo's `review` skill.
