...
- **When the CI command cannot run here, the honest output is "unverified" — not a substitute reported as verified.** Trunk's daemon needs process control the sandbox denies, so `trunk check` is unavailable in-session. Reaching for its cached binaries gets you *one* linter: `prettier -w` formats and reports clean while `markdownlint`, which CI also runs, never executes. Formatted is not linted, and `fmt` repairs almost no lint rules. Name which checks ran and which did not.
