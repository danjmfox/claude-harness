...
- **PATH keeps `~/.claude/bin` ahead of `~/.local/bin`** (`zsh/runcoms/zprofile:26`, `settings.json` `env.PATH`), so bare `des-roadmap` is correct and needs no prefix. That ordering is the whole protection: pre-3.22.1, the `~/.local/bin/des-*` scripts were wheel-generated and could not import `des` — the wheel declared console-script entry points for a package it did not ship — and they still failed when called by absolute path, or from a process not inheriting this PATH. As of 3.22.1 (below) the entry points are gone from the wheel entirely, so on an upgraded machine those `~/.local/bin/des-*` are dangling symlinks instead — same PATH protection, different failure shape if bypassed.
...
- **nWave v3.22.1** is a maintenance release: verified via `git diff v3.22.0..v3.22.1 -- ':!docs' ':!README.md'` (file count held at 1327, no silent tree shift). The only non-version-bump change is `pyproject.toml` dropping the `[project.scripts]` entries for `des-log-phase`, `des-commit`, `des-init-log`, `des-verify-integrity`, `des-roadmap`, `des-health-check` — the wheel stops declaring those console scripts at all. On a machine with pre-existing `~/.local/bin/des-*` symlinks (from an older install), upgrading to 3.22.1 leaves them dangling rather than merely import-broken, since the pipx venv no longer ships the binaries they pointed at. No behavior change here: `~/.claude/bin/des-*` (see PATH note above) is what actually resolves and is regenerated fresh by every `nwave-ai install`.
...
- **Delegation Budget:** Delegate for two reasons — large independent parallelizable work, or an independent-perspective gate against an external standard (`*-reviewer` agents, DoR/AC conformance). Do not spawn a subagent to re-check your own work for correctness. Don't delegate what you can finish in a handful of tool calls. One agent beats several. Treat every subagent report as a hypothesis — verify its claims against the cited `file:line` before acting on them. When a claim about *our own* files turns out false, correct the artifact in the same turn, not just in conversation: saying it in the transcript does not update the deliverable, and a wrong claim left in a doc ships.
