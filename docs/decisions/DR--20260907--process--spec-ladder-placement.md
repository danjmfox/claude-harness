...
Size each layer at `nw-new` time to a roadmap-sized slice — "add rule X with its acceptance test,"
not "the whole feature." Run DISTILL and DELIVER exactly as nwave-ai specifies for each slice; the
...
- Gets real skip/unskip-flavoured decomposition — an earlier slice's acceptance test can genuinely
  sit authored-but-unimplemented until the next slice lands — using only tooling that exists today.
...
As a direct consequence, `ship`'s draft-PR step was generalised (not scoped to recognized stack
...
Revisit Option 2 if Option 3 proves insufficient in practice — but only through its own decision
record and, likely, an upstream nwave-ai proposal. Do not fork or patch `nw-deliver` locally as a
