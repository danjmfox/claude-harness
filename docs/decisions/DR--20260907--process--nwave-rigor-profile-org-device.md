...
- **Rejected.** `review_enabled: false` directly contradicts `ENGINEERING-DEFAULTS.md`'s existing
  rule — *"Reviewers always run... never skip a wave's reviewer gate to 'stay lean' — running it
  is the lean choice"* — verbatim anticipating and forbidding this exact preset. Dropping the
  `COMMIT` TDD phase also risks breaking `nw-deliver`'s own Phase 2 gate ("all steps reach
  `COMMIT`/`PASS`") and `buildy`'s `watch` terminal condition (commit count == `STEPS`), though this
  was not verified against `nw-deliver`'s gate logic directly.

### Option 2: Custom profile, `agent_model: haiku`, `reviewer_model: haiku`

Keeps `review_enabled: true` (unlike Option 1), but downgrades the *implementer* model to Haiku.

- **Rejected.** `agent_model` drives the crafter/architect/acceptance-designer roles — the highest-
  stakes reasoning in the pipeline, not a minor setting. A weaker implementer raises the odds the
  (already-mandatory) reviewer flags something, triggering rework — which costs more net tokens
  than doing it right once, the same logic `ENGINEERING-DEFAULTS.md` already uses to justify
  mandatory reviewers. No spend data existed to justify this as a necessary cut; it was invented
  cost pressure, not a verified one.
...
shows `standard` genuinely threatens the £1500/month cap, revisit — but only with that evidence in
hand, not by preemptively downgrading `agent_model` again.
